...
import { toTimestampTZ, formatDateTimeBR } from '@/lib/dateUtils';
      ...
      {isRH && status === 'documentos_em_analise' && (
        <Card>
          <CardContent className="p-4 space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Links gerados automaticamente. O candidato envia cada documento individualmente.
            </p>
                ...
                <div key={c.id} className="border border-border rounded-lg p-3 space-y-2">
                  ...
                  {link && link !== 'EXISTS' ? (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">Link válido.</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
            ...
            {Object.keys(generatedLinks).some(k => k.startsWith('DOCUMENTS-')) && (
              <>
                <div className="flex items-center gap-3 pt-2">
                  <Checkbox id="docs-confirmed" checked={docsConfirmed} onCheckedChange={(checked) => setDocsConfirmed(!!checked)} />
                  <Label htmlFor="docs-confirmed" className="text-sm">Confirmar recebimento de todos os documentos</Label>
                </div>
                <Button onClick={() => handleStatusChange('aguardando_exame')} disabled={!docsConfirmed} className="gap-2 w-full" size="sm">
      ...
      {isRH && (status === 'aguardando_exame' || status === 'exame_realizado') && (
        <Card>
          <CardContent className="p-4 space-y-3">
            ...
            <p className="text-xs text-muted-foreground">Selecione a clínica, data e hora. Após a data/hora, registre o resultado.</p>
            {approvedCandidates.map((c: any) => (
        ...
        <Card>
          <CardContent className="p-4 space-y-3">
            ...
            {approvedCandidates.map((c: any) => {
              ...
              return (
                <SignatureSection
                  key={c.id} admissionId={id!} candidateId={c.id} candidateName={c.nome}
                  link={link && link !== 'EXISTS' ? link : undefined} linkExists={link === 'EXISTS'}
        ...
        <Card className="border-primary/30 bg-primary/5">
          <CardContent className="p-4 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {approvedCandidates.map((c: any) => (
                <WelcomePdfGenerator
                  key={c.id} candidateName={c.nome} cargoFuncao={req.cargo_funcao} admissionId={id!}
                  defaultLocal={req.local_contratacao} defaultResponsavel={req.gestor_responsavel} dataPrevistaInicio={req.data_prevista_inicio}
                />
              ))}
      ...
      <Dialog open={showAddCandidate} onOpenChange={(open) => { if (!open) { setShowAddCandidate(false); setEditCandidateId(null); setCandidateForm({ nome: '', cpf: '', telefone: '', email: '', cidade: '' }); } }}>
        <DialogContent>
          ...
          <div className="space-y-3">
            <div className="space-y-1.5"><Label>Nome *</Label><Input value={candidateForm.nome} onChange={e => setCandidateForm((p: any) => ({ ...p, nome: e.target.value }))} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label>CPF</Label>
                <Input value={candidateForm.cpf} onChange={e => setCandidateForm((p: any) => ({ ...p, cpf: maskCPF(e.target.value) }))} placeholder="000.000.000-00" maxLength={14} />
                {cpfError && <p className="text-xs text-destructive">{cpfError}</p>}
              </div>
              <div className="space-y-1.5">
                <Label>Telefone</Label>
                <Input value={candidateForm.telefone} onChange={e => setCandidateForm((p: any) => ({ ...p, telefone: maskPhone(e.target.value) }))} placeholder="(00) 00000-0000" maxLength={15} />
              </div>
            </div>
            <div className="space-y-1.5"><Label>Email</Label><Input type="email" value={candidateForm.email} onChange={e => setCandidateForm((p: any) => ({ ...p, email: e.target.value }))} /></div>
            <div className="space-y-1.5"><Label>Cidade</Label><Input value={candidateForm.cidade} onChange={e => setCandidateForm((p: any) => ({ ...p, cidade: e.target.value }))} /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setShowAddCandidate(false); setEditCandidateId(null); }}>Cancelar</Button>
            <Button onClick={handleAddCandidate} disabled={!candidateForm.nome || !!cpfError}>
...
// ===== HELPER COMPONENTS REUSED FROM ORIGINAL PAGE =====
...
function ExamSection({ candidateId, candidateName, admissionId, currentStatus, onAdvance, onExamResultRegistered }: {
  ...
  useRealtimeSubscription({
    ...
    enabled: !!candidateId,
    tables: [{ table: 'medical_exams', filter: `candidate_id=eq.${candidateId}`, queryKeys: [['medical_exam', candidateId]] }],
  });
  ...
  const handleCreateExam = async () => {
    if (!clinicName || !examDate || !examTime) { toast({ title: 'Preencha clínica, data e hora', variant: 'destructive' }); return; }
    if (examDate < minDateToday()) { toast({ title: 'A data do exame deve ser hoje ou uma data futura.', variant: 'destructive' }); return; }
    const scheduledAt = toTimestampTZ(examDate, examTime);
    const { error } = await supabase.from('medical_exams').insert({
      candidate_id: candidateId, clinic_id: null, clinic_name: clinicName, scheduled_at: scheduledAt, restrictions: examNotes || null,
    } as any);
    if (error) toast({ title: 'Erro', description: error.message, variant: 'destructive' });
    else { toast({ title: 'Exame agendado' }); qc.invalidateQueries({ queryKey: ['medical_exam', candidateId] }); }
  };
  ...
  const handleExamResult = async (result: string) => {
    ...
    if (error) toast({ title: 'Erro', description: error.message, variant: 'destructive' });
      ...
      qc.invalidateQueries({ queryKey: ['medical_exam', candidateId] });
      if (currentStatus === 'aguardando_exame') onExamResultRegistered?.();
    }
  };
      ...
      {exam ? (
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            {exam.status === 'aguardando' && !isExamPast && (
              <div className="mt-1 space-y-0.5">
                <p className="flex items-center gap-1 text-xs text-[hsl(var(--status-info-foreground))]"><Clock className="w-3 h-3" /> Aguardando data/hora do exame</p>
                {getTimeRemaining() && <p className="text-[10px] text-muted-foreground">{getTimeRemaining()}</p>}
              </div>
            )}
            {isExamPast && exam.status === 'aguardando' && (
              <>
                <p className="flex items-center gap-1 text-xs text-primary font-medium mt-1"><CheckCircle className="w-3 h-3" /> Liberado — registre o resultado</p>
          ...
          <div className="space-y-1 pt-2 border-t border-border">
            ...
            {!isExamPast && <p className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="w-3 h-3" /> Aguarde a data/hora do exame para prosseguir.</p>}
            {isExamPast && !examResolved && <p className="text-xs text-muted-foreground flex items-center gap-1"><AlertTriangle className="w-3 h-3 text-destructive" /> Registre o resultado do exame antes de avançar.</p>}
            {examResolved && !hasExamAttachment && <p className="text-xs text-destructive flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> Anexe o exame admissional antes de avançar.</p>}
            <Button onClick={onAdvance} disabled={!canAdvance} className="gap-2 w-full" size="sm">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1.5">
            ...
            <DynamicCategorySelect module="admissions" fieldKey="clinic_name" value={clinicName} onValueChange={setClinicName} placeholder="Selecione ou adicione" />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1.5">
              <Label className="text-xs">Data *</Label>
              <Input type="date" value={examDate} onChange={e => setExamDate(e.target.value)} min={minDateToday()} />
              {examDate && examDate < minDateToday() && <p className="text-xs text-destructive">A data do exame deve ser hoje ou uma data futura.</p>}
            </div>
            <div className="space-y-1.5"><Label className="text-xs">Hora *</Label><Input type="time" value={examTime} onChange={e => setExamTime(e.target.value)} /></div>
          </div>
          <div className="space-y-1.5"><Label className="text-xs">Observações</Label><Input value={examNotes} onChange={e => setExamNotes(e.target.value)} placeholder="Opcional" /></div>
...
function SignatureSection({ admissionId, candidateId, candidateName, link, linkExists, onCopyLink }: { admissionId: string; candidateId: string; candidateName: string; link?: string; linkExists?: boolean; onCopyLink: () => void; }) {
  const { toast } = useToast();
  const qc = useQueryClient();
  ...
  const handleAdminUpload = async (file: File, docKey: string) => {
    ...
    if (!allowed.includes(file.type as any)) { toast({ title: 'Tipo de arquivo não permitido', variant: 'destructive' }); return; }

    const isValidMagicNumber = await validateFileMagicNumber(file, allowed as any);
    if (!isValidMagicNumber) { toast({ title: 'Arquivo inválido', variant: 'destructive' }); return; }
    ...
    if (error) toast({ title: 'Erro no upload', description: error.message, variant: 'destructive' });
      ...
      await supabase.from('admission_files').insert({ admission_request_id: admissionId, candidate_id: candidateId, file_type: docKey, storage_path: path, original_filename: sanitized, uploaded_by: 'ADMIN', link_type: 'SIGNATURE' } as any);
  ...
  return (
    <div className="border border-border rounded-lg p-3 space-y-3">
        ...
        {ADMIN_SIGNATURE_DOCS.map(doc => {
          ...
          return (
            <div key={doc.key} className="border border-border rounded-lg p-2.5 space-y-1.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <FileText className="w-3.5 h-3.5 text-muted-foreground" /><span className="text-xs font-medium text-foreground">{doc.label}</span>
                </div>
                <div className="flex items-center gap-2">
                  {doc.optional && (
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] text-muted-foreground">Aplicável?</span>
                      <Switch checked={!isSkipped} onCheckedChange={v => setSkipped(prev => ({ ...prev, [doc.key]: !v }))} className="scale-75" />
                    </div>
                  )}
                  {existingFile ? <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium status-approved"><CheckCircle className="w-3 h-3" /> Anexado</span>
                  : isSkipped ? <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-muted text-muted-foreground">N/A</span>
                ...
                <>
                  {existingFile && (
                    <div className="flex items-center justify-between text-xs bg-muted/30 rounded px-2 py-1">
                      <span className="flex items-center gap-1 truncate"><FileText className="w-3 h-3 text-muted-foreground" />{existingFile.original_filename || 'doc'}</span>
                      <Button variant="ghost" size="sm" className="h-6 px-1.5" onClick={() => handleDownload(existingFile.storage_path)}><Download className="w-3 h-3" /></Button>
                    </div>
                  )}
                  <div className="flex items-center gap-2">
                    <Input type="file" accept=".pdf" disabled={isUploading} onChange={e => { if (e.target.files?.[0]) handleAdminUpload(e.target.files[0], doc.key); }} className="text-xs h-8" />
        ...
        <div className="space-y-1">
          ...
          {signedFiles.map(f => (
            <div key={f.id} className="flex items-center justify-between text-xs bg-primary/5 rounded px-2 py-1">
              <span className="flex items-center gap-1 truncate"><CheckCircle className="w-3 h-3 text-primary" /> {f.original_filename || 'doc'}</span>
              <Button variant="ghost" size="sm" className="h-6 px-1.5" onClick={() => handleDownload(f.storage_path)}><Download className="w-3 h-3" /></Button>
            </div>
          ))}
        </div>
      )}
      ...
      ) : linkExists ? <div className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-primary" /><span className="text-xs text-muted-foreground">Link já gerado.</span></div>
      : <div className="flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin text-primary" /><span className="text-xs text-muted-foreground">Gerando...</span></div>}
    </div>
  );
}
...
function CreateCollaboratorFromAdmissionButton({ admissionId, candidates, cargoFuncao, worksite, uniformSizes }: { admissionId: string; candidates: any[]; cargoFuncao: string; worksite: string; uniformSizes?: Record<string, string> }) {
  ...
  const handleCreate = async () => {
    for (const c of candidates) {
      await createCollab.mutateAsync({
        full_name: c.nome, cpf: c.cpf || null, role_name: cargoFuncao, worksite,
        admission_request_id: admissionId, uniform_sizes: uniformSizes || {},
      });
    }
    setDone(true);
  };
  ...
  return (
    <Button variant="outline" size="sm" className="gap-1.5" onClick={handleCreate} disabled={createCollab.isPending || candidates.length === 0}>
      {createCollab.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <HardHat className="w-4 h-4" />} Criar Colaborador p/ EPIs
    </Button>
  );
}
...
function StartEpiDeliveryButton({ admissionId }: { admissionId: string }) {
  ...
  useEffect(() => {
    supabase.from('collaborators').select('id').eq('admission_request_id', admissionId).limit(1).then(({ data }) => {
  ...
  return (
    <Button variant="outline" size="sm" className="gap-1.5" onClick={() => navigate(`/epis/deliveries?collaboratorId=${collabId}`)}>

  ...
  return useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from('epi_deliveries')
        .select('*, collaborator:collaborators(id, full_name, cpf, role_name), epi_item:epi_items(id, code, name, category, ca_number), delivered_by:profiles!epi_deliveries_delivered_by_user_id_fkey(id, full_name), sector:sectors(id, name)')

...
import { useDashboardQueue } from '@/modules/dashboard/hooks/useDashboardQueue';
import type { QueueItem } from '@/modules/dashboard/queries/dashboardQueueLoader';
...
const COMPLETION_LABELS: Record<string, string> = {
  ...
  confirmar_horas: 'Confirmar horas',
  concluir_revisao: 'Concluir revisão',
  concluir_triagem: 'Concluir triagem',
  concluir_processamento_rh: 'Concluir processamento de RH',
  concluir: 'Concluir etapa',
};
...
function PendingApprovalCard({
  ...
  item: QueueItem;
  processAction: ReturnType<typeof useProcessApproval>;
  onReasonAction: (moduleKey: string, entityId: string, type: 'reject' | 'return') => void;
}) {
  const navigate = useNavigate();
  const moduleKey = item.module_code ?? '';
  const { data: actionContext, isLoading, error } = useApprovalContext(item.reference_id, moduleKey);
  const completionAction = actionContext?.meta.step_action ?? undefined;
  const allowed = actionContext?.permissions.allowed_actions ?? [];
  const route = approvalModuleDetailRoute(moduleKey, item.reference_id);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <Badge variant="secondary" className="text-xs">{item.module_name}</Badge>
              <Badge variant="outline" className="text-[10px]">
                Etapa {actionContext?.flow.current_step || item.current_step_order || '—'}
                {actionContext?.flow.total_steps ? ` de ${actionContext.flow.total_steps}` : ''}
              </Badge>
            </div>
            <div className="flex items-center gap-x-2 gap-y-1 text-sm text-muted-foreground flex-wrap min-w-0">
              <User className="w-3.5 h-3.5 shrink-0" />
              <span className="font-medium text-foreground/90 truncate">{actionContext?.requester.name || item.requester_name || 'Desconhecido'}</span>
              <span className="hidden sm:inline">·</span>
              <Clock className="w-3.5 h-3.5 shrink-0" />
              <span className="shrink-0">{formatDistanceToNow(new Date(item.created_at), { addSuffix: true, locale: ptBR })}</span>
            </div>
            {actionContext?.flow.current_step_name && (
              <p className="text-xs text-muted-foreground mt-2">{actionContext.flow.current_step_name}</p>
            )}
            {actionContext?.meta.sla_deadline && (
              <p className="text-xs text-muted-foreground mt-1">SLA: {new Date(actionContext.meta.sla_deadline).toLocaleString('pt-BR')}</p>
            )}
            {!isLoading && !completionAction && actionContext?.meta.waiting_label && (
              <p className="text-xs text-muted-foreground mt-2">{actionContext.meta.waiting_label}</p>
            )}
            {error && <p className="text-xs text-destructive mt-2">Action Context indisponível.</p>}
          </div>
          <div className="flex gap-2 shrink-0 flex-wrap items-center">
            {route && (
              <Button size="sm" variant="ghost" onClick={() => navigate(route)} className="gap-1">
                <ExternalLink className="w-4 h-4" /> Detalhes
              </Button>
            )}
            {completionAction && allowed.includes(completionAction) && (
              <Button
                size="default"
                onClick={() => processAction.mutate({ moduleKey, entityId: item.reference_id, action: 'approve', completionAction })}
                disabled={processAction.isPending}
                className="gap-1.5 font-semibold"
              >
                <CheckCircle2 className="w-4 h-4" /> {COMPLETION_LABELS[completionAction] || completionAction}
              </Button>
            )}
            {allowed.includes('rejeitar') && (
              <Button size="default" variant="destructive" onClick={() => onReasonAction(moduleKey, item.reference_id, 'reject')} disabled={processAction.isPending} className="gap-1.5 font-semibold">
                <XCircle className="w-4 h-4" /> Reprovar
              </Button>
            )}
            {allowed.includes('devolver') && (
              <Button size="default" variant="outline" onClick={() => onReasonAction(moduleKey, item.reference_id, 'return')} disabled={processAction.isPending} className="gap-1.5">
...
export default function MyApprovalsTab() {
  ...
  // A fila operacional vem exclusivamente de get_my_approval_queue().
  const pendingRequestIds = new Set(myPending.map(item => item.id));
  const otherApprovals = sortedApprovals.filter((a: any) => !pendingRequestIds.has(a.id));
          ...
          <div className="space-y-3">
            {myPending.map(item => (
              <PendingApprovalCard
                ...
                onReasonAction={(moduleKey, entityId, type) => {
                  setActionDialog({ moduleKey, entityId, type });
                  setActionReason('');
                }}
              />
            ))}

...
export default function CandidateDetailPage() {
  ...
  const handleRegistration = async (field: string, value: boolean) => {
    if (!candidateId) return;
    if (!registration) {
      await (supabase.from('system_registrations') as any).insert({ candidate_id: candidateId, [field]: value });
    } else {
      await (supabase.from('system_registrations') as any).update({ [field]: value }).eq('id', registration.id);
    }
    refetchReg();
  };

...
-- View for admission list items with candidate and document status aggregated
CREATE OR REPLACE VIEW public.vw_admissions_list_items AS
SELECT
  ar.id,
  ar.cargo_funcao,
  ar.centro_custo,
  ...
  -- Primary candidate: latest approved or first active candidate
  c_primary.nome AS candidato_nome,
  c_primary.id AS candidato_id,
  -- Candidate count
  COALESCE(c_stats.total_candidatos, 0) AS total_candidatos,
  -- Document status semaphore: 'completo', 'parcial', 'pendente', 'sem_candidato'
  CASE

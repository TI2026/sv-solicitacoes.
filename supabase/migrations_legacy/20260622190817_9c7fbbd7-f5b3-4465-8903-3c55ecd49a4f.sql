...
BEGIN
  ...
  IF _to_status = 'enviado' THEN
    INSERT INTO public.notifications (user_id, title, message, metadata)
    SELECT DISTINCT dest_uid, title_text, body_text, meta_json
    FROM (
      SELECT ur.user_id AS dest_uid
      FROM public.user_roles ur
      WHERE ur.role IN ('diretoria', 'administrativo') AND ur.user_id != _uid
      UNION
      SELECT ura.user_id AS dest_uid
      FROM public.user_role_assignments ura
      JOIN public.roles r ON r.id = ura.role_id
    ...
    CROSS JOIN (
      SELECT
        'Nova solicitação recebida'::text AS title_text,
        format('Nova solicitação de %s aguardando encaminhamento', _req.type)::text AS body_text,
        jsonb_build_object('entity_type', 'fuel_requests', 'entity_id', _request_id, 'status', _to_status) AS meta_json
    ) AS payload;
  END IF;
  ...
  IF _to_status = 'em_aprovacao' THEN
    ...
    FROM (
      SELECT ur.user_id AS dest_uid
      FROM public.user_roles ur
      WHERE ur.role = 'diretoria' AND ur.user_id != _uid
      UNION
      SELECT ura.user_id AS dest_uid
      FROM public.user_role_assignments ura
      JOIN public.roles r ON r.id = ura.role_id
      WHERE (r.key = 'diretoria' OR r.is_master = TRUE)
        AND ura.user_id != _uid
    ) AS recipients
    CROSS JOIN (
      SELECT
        'Solicitação aguardando aprovação'::text AS title_text,
        format('Solicitação de %s encaminhada para sua aprovação', _req.type)::text AS body_text,
        jsonb_build_object('entity_type', 'fuel_requests', 'entity_id', _request_id, 'status', _to_status) AS meta_json
    ) AS payload;
  END IF;
  ...
  IF _to_status = 'em_revisao_admin' THEN
    ...
    CROSS JOIN (
      SELECT
        'Fotos enviadas para revisão'::text AS title_text,
        'Colaborador enviou hodômetro e nota fiscal para conferência'::text AS body_text,
        jsonb_build_object('entity_type', 'fuel_requests', 'entity_id', _request_id, 'status', _to_status) AS meta_json
    ) AS payload;
  END IF;

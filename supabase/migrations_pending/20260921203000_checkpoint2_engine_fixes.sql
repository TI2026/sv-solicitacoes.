-- CHECKPOINT 2 — correções dos defeitos de homologação (D-01 a D-04).
-- Migration incremental PENDENTE: validada apenas no banco LOCAL de homologação.
-- Não foi aplicada em produção; a aplicação exige autorização explícita.
-- Não altera templates, steps nem a semântica do Motor V2.

-- =====================================================================
-- D-01 — Operação de Compras: visibilidade alinhada à RLS da tabela
-- purchases. Financeiro/Compras já possuem SELECT por policy e já eram
-- autorizados pelo executor nas ações operacionais; _engine_can_view
-- negava antes, devolvendo "registro não encontrado". O acesso continua
-- restrito ao módulo Compras — nenhum outro módulo é afetado.
-- =====================================================================
...
BEGIN
  ...
  -- Fase operacional de Compras (OC, pagamento, entrega, conclusão).
  IF v_module = 'compras'
...
-- =====================================================================
-- D-03 — status_history operacional: entity_type canônico (tabela da
-- entidade) e sem entradas duplicadas quando o motor e o trigger da
-- tabela registram a mesma transição na mesma transação.
-- audit_logs continua intacto (evento técnico).
-- =====================================================================
...
BEGIN
  IF NEW.from_status IS NOT DISTINCT FROM NEW.to_status THEN
    RETURN NULL;  -- não é transição real
  END IF;
  ...
  -- A mesma transição da mesma entidade só é registrada uma vez por
  -- transação, mesmo que motor e trigger da tabela a reportem.
  IF EXISTS (
...
-- =====================================================================
-- D-02 — notificações idempotentes por destinatário/evento.
-- =====================================================================
...
-- Solicitante acompanha cada ativação de etapa (envio, reenvio e avanço),
-- na mesma transação do workflow e sem duplicar em retry/concorrência.
...
-- =====================================================================
-- D-04 — exclusão física não pode deixar fluxo/etapas órfãos.
-- Workflow ativo bloqueia o DELETE; fluxo encerrado é removido junto.
-- =====================================================================
...
-- =====================================================================
-- D-03 (parte 2) — trilha operacional por solicitação em approval_history,
-- sem substituir audit_logs.
-- =====================================================================
...
BEGIN
  ...
  -- Trilha operacional da solicitação (complementar ao audit_logs).

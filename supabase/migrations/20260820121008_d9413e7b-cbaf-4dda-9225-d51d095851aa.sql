...
BEGIN
  CASE p_module_code
    WHEN 'compras' THEN
      UPDATE public.purchases SET status = p_new_status, updated_at = now() WHERE id = p_entity_id;
    WHEN 'abastecimento', 'diaria', 'reembolso' THEN
      UPDATE public.fuel_requests SET status = p_new_status::public.fuel_status, updated_at = now() WHERE id = p_entity_id;
    WHEN 'admissoes', 'admissions' THEN
      UPDATE public.admission_requests SET status = p_new_status::public.admission_status, updated_at = now() WHERE id = p_entity_id;
    WHEN 'desligamentos' THEN
      UPDATE public.termination_requests SET status = p_new_status::public.termination_status, updated_at = now() WHERE id = p_entity_id;
    ELSE
...
BEGIN
  ...
  IF p_action = 'approve' THEN
    ...
    ELSE
      ...
      CASE v_module_code
        ...
        WHEN 'abastecimento' THEN v_entity_status := 'aprovado';
        WHEN 'diaria'        THEN v_entity_status := 'aprovado';
        WHEN 'reembolso'     THEN v_entity_status := 'aprovado';
        WHEN 'admissoes'     THEN v_entity_status := 'documentos_em_analise';
        WHEN 'admissions'    THEN v_entity_status := 'documentos_em_analise';
        WHEN 'desligamentos' THEN v_entity_status := 'aprovado';
        ELSE v_entity_status := 'aprovado';
      END CASE;

      UPDATE public.approval_requests
      SET status = v_new_status, ended_at = now(), updated_at = now()
      WHERE id = p_approval_request_id;
  ...
  VALUES (v_module_code, v_module_code, v_req.reference_id, 'em_aprovacao', COALESCE(v_entity_status, v_new_status), v_uid);

  INSERT INTO public.audit_logs (user_id, action, entity_type, entity_id, details)
  VALUES (
    v_uid, 'APPROVAL_' || upper(p_action), v_module_code, v_req.reference_id,
    jsonb_build_object(

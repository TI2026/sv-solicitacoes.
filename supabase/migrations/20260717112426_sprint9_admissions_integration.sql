-- ============================================================
-- SPRINT 9: Integracao do Modulo Admissoes ao Approval Engine
--
-- Adiciona sincronizacao do dominio admissions na funcao
-- process_approval_action, seguindo o mesmo padrao de compras
...
BEGIN
  IF p_action NOT IN (''approve'',''reject'',''return'') THEN
    RETURN jsonb_build_object(''code'',''ENGINE-400'',''message'',''Acao invalida'');
  END IF;
  ...
  EXCEPTION
    WHEN lock_not_available THEN
      RETURN jsonb_build_object(''code'',''FLOW-008'',''message'',''Outra operacao esta processando esta solicitacao.'');
    WHEN no_data_found THEN
      RETURN jsonb_build_object(''code'',''ENGINE-404'',''message'',''Solicitacao nao encontrada'');
  END;

  IF _req.ended_at IS NOT NULL THEN 
    RETURN jsonb_build_object(''code'',''ENGINE-403'',''message'',''Fluxo ja encerrado''); 
  END IF;
  
  IF _req.current_approver_user_id != _uid THEN
    RETURN jsonb_build_object(''code'',''AUTH-009'',''message'',''Voce nao e o aprovador da etapa atual'');
  END IF;
  ...
  IF p_action = ''reject'' THEN
    IF _flow.require_rejection_reason AND (p_comments IS NULL OR trim(p_comments)='''') THEN
      RETURN jsonb_build_object(''code'',''ENGINE-400'',''message'',''E obrigatorio informar o motivo da recusa'');
    END IF;
    _new := ''rejected'';
  ...
  ELSIF p_action = ''return'' THEN
    IF NOT _flow.allow_return_for_adjustment THEN
      RETURN jsonb_build_object(''code'',''ENGINE-403'',''message'',''Este fluxo nao permite devolucao'');
    END IF;
    IF p_comments IS NULL OR trim(p_comments)='' '' THEN
      RETURN jsonb_build_object(''code'',''ENGINE-400'',''message'',''E obrigatorio informar o motivo da devolucao'');
    END IF;
    ...
    IF COALESCE(_flow.return_mode,''requester'') = ''previous_step'' THEN
      ...
      IF _prev.id IS NOT NULL THEN
        ...
        IF _flow.notify_next_approver THEN
          ...
          VALUES (_prev.approver_user_id,''Solicitacao devolvida para reanalise'',
  ...
  ELSIF p_action = ''approve'' THEN
    ...
    IF _next.id IS NOT NULL THEN
      ...
      IF _flow.notify_next_approver THEN
        INSERT INTO notifications (user_id,title,message,metadata)
        VALUES (_next.approver_user_id,''Nova aprovacao pendente'',
          ''Solicitacao aguardando sua aprovacao'',
          jsonb_build_object(''entity_type'',''approval_request'',''entity_id'',p_approval_request_id));
      END IF;
    ELSE
      _new := ''approved'';
  ...
  IF _req.requester_user_id != _uid THEN
    INSERT INTO notifications (user_id,title,message,metadata) VALUES (_req.requester_user_id,
      CASE p_action
        WHEN ''approve'' THEN CASE WHEN _new=''approved'' THEN ''Solicitacao aprovada'' ELSE ''Etapa aprovada'' END
        WHEN ''reject'' THEN ''Solicitacao recusada''
        WHEN ''return'' THEN ''Solicitacao devolvida para correcao''
      END,
      CASE p_action
        WHEN ''approve'' THEN ''Sua solicitacao avancou no fluxo''
        WHEN ''reject'' THEN COALESCE(''Motivo: ''||p_comments,''Recusada'')
        WHEN ''return'' THEN COALESCE(''Motivo: ''||p_comments||'' corrija e reenvie'',''Devolvida para correcao'')
      END,
      jsonb_build_object(''entity_type'',''approval_request'',''entity_id'',p_approval_request_id));
  END IF;

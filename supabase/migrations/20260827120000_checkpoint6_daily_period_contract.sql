-- CHECKPOINT 6 — Diárias por período + proofs enforced by the V2 executor.
-- Published history remains untouched. `daily_value` is the frozen unit rate
-- and `valor` remains the authoritative financial total used by existing code.
...
-- Notification/queue links must use the same canonical process routes.
...
-- New rows/changes are checked without rewriting compatible historical rows.
ALTER TABLE public.fuel_requests
...
DECLARE
  ...
  v_quantity integer;
BEGIN
  -- Restore/seed and SECURITY DEFINER motor operations run as an owner.
  ...
  IF v_type = 'diaria' THEN
    ...
    NEW.data_abastecimento := v_start_date; -- legacy/report compatibility
    NEW.daily_quantity := v_quantity;       -- never trust the client
    NEW.valor := round(v_quantity * NEW.daily_value, 2); -- authoritative total
  ELSE
...
-- Preserve the current public executor and add only precondition checks. The
-- previous executor remains responsible for authorization, locks, transitions,
...
BEGIN
  ...
  IF v_module = 'diaria' AND v_action = 'enviar' THEN
    -- Historical rows with only `data_abastecimento` stay readable, but every
    -- new/submitted request must carry the frozen period contract.
...
-- Draft editing is a data operation gated by Action Context, not a workflow
-- action.  The backend recalculates quantity and total before persisting.

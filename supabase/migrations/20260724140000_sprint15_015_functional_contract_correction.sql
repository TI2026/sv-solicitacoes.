-- ============================================================
-- SPRINT 15.1: Migration 015 - Functional Contract Correction
-- ============================================================
...
DECLARE
  ...
  _current_uid uuid := auth.uid();
  _real_requester uuid;
  _table_name text;
BEGIN
  -- 1. Rejeitar auth.uid() nulo
  IF _current_uid IS NULL THEN
    RETURN jsonb_build_object('error', 'Usuário não autenticado');
  END IF;

  -- 2 e 3. Não confiar em requester_user_id vindo do cliente.
  -- 8 e 9. Permitir start_on_behalf (permissão administrativa).
  IF p_requester_user_id <> _current_uid THEN
    IF NOT (public.has_role(_current_uid, 'master'::app_role) OR public.has_role(_current_uid, 'administrativo'::app_role)) THEN
      RETURN jsonb_build_object('error', 'Sem permissão para iniciar fluxo em nome de outro usuário');
    END IF;
    -- 12. Registrar auditoria de impersonation
    INSERT INTO public.audit_logs (user_id, action, entity_type, entity_id, details)
    VALUES (_current_uid, 'START_ON_BEHALF', 'approval_requests', p_reference_id, jsonb_build_object('target_user', p_requester_user_id, 'module', p_module_code));
  END IF;

  -- 4 e 5. Validar que a entidade reference_id realmente existe no módulo informado
  -- 6 e 7. Identificar o requester real na tabela de negócio e confirmar que requester_user_id da entidade é o solicitado
  _table_name := CASE p_module_code
    ...
    -- 'diarias' e 'reembolso' usam a mesma logica se existirem, mas o sistema tem 'daily_allowance_requests' e 'refund_requests'? 
    -- Nao, foram mesclados ou tem nome diferente. Na base local eu vi purchases, fuel_requests, admission_requests, termination_requests.
    -- Vamos deixar genérico mas validar existencialmente
    ELSE NULL
  END;

  IF _table_name IS NOT NULL THEN
    BEGIN
      EXECUTE format('SELECT requester_user_id FROM public.%I WHERE id = $1 LIMIT 1', _table_name)
        INTO _real_requester
        USING p_reference_id;
    EXCEPTION WHEN undefined_table THEN
      -- Fallback se a tabela não existir, apenas avisa mas não bloqueia hard caso seja um modulo dinamico.
      -- O ideal é falhar, mas vamos garantir as 4 tabelas principais.
    END;

    IF _real_requester IS NULL THEN
      RETURN jsonb_build_object('error', 'Registro de solicitação não encontrado no módulo fornecido');
    END IF;

    IF _real_requester <> p_requester_user_id THEN
      RETURN jsonb_build_object('error', 'Requester mismatch: O usuário fornecido não é o criador da solicitação');
    END IF;
  END IF;

  -- 10 e 11. Bloquear fluxo duplicado ativo (usando FOR UPDATE SKIP LOCKED ou apenas EXISTS)
  IF EXISTS (
    SELECT 1 FROM public.approval_requests 
    WHERE reference_id = p_reference_id AND status NOT IN ('approved', 'rejected', 'cancelled')
  ) THEN
    RETURN jsonb_build_object('error', 'Já existe um fluxo ativo para esta solicitação');
  END IF;
